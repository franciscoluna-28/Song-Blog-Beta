"use strict";
//dom setup
const buttonDiv = document.getElementById("pageButtons");
const domButtons = buttonDiv === null || buttonDiv === void 0 ? void 0 : buttonDiv.getElementsByTagName("a");
const myButtons = Array.from(domButtons);
...
//maps
let myIDs = availableTracks.map(track => track.id);
const myNames = availableTracks.map(track => track.name);
const myLinks = availableTracks.map(track => track.link);
//random ID generator 
...
myButtons.forEach(button => {
    const availableID = shuffledArray.shift();
    const randomSet = availableTracks.find(Element => Element.id === availableID); //Matching an ID to the 
    //availableTracks array to get the metadata
    //once we get the metadata is easy to access to it using dom properties
    button.textContent = randomSet === null || randomSet === void 0 ? void 0 : randomSet.name; //Dom text content
    button.setAttribute("href", randomSet === null || randomSet === void 0 ? void 0 : randomSet.link); //Dom link 
});
/* //global IDs
